 ...
 */
class Generalregistration extends BaseController
{
    /**
    ...
    function index()
    {
        ...
        if ($this->input->post()) {
            ...
            $studentCount =  $this->job_model->getGeneralCandidateCount();
            $data = array(
                'name' => $name,
                'mobile' => $mobile,
                'email' => $email,
                ...
                'uid' => 'GTTCST' . date('Y').sprintf("%'.05d", $studentCount+1),
                'type' => 2,
            );

            $student_id = $this->job_model->addCandidate($data);
            redirect('/generalregistration/thanks/'.$student_id);
        }
        ...
        $this->load->view("jobs/generalregister",  $data);
    }
    function thanks($id)
    {
        ...
        $this->load->view("jobs/thanks1",  $data);
    }
